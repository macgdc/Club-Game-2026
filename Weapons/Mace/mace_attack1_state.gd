...
func enter(actor: Node) -> void:
	assert(actor is Player, "Mace Attack 1 cannot initialize since actor is not Player type it is: " + str(actor))
...
func physics_update(delta: float) -> void:
	if attack_buffer_timer.is_stopped():	
	...
	if Input.is_action_just_pressed(input_action) and not attack_buffer_timer.is_stopped():
		#print("Attack is being buffered")
		is_attack_buffered = true
		state_transition.emit("attack2")
...
func attack() -> void:
	...
	mace_skin.attack1()
	player_animator.play("player_anims/Mace Attack")
	#print("ATTACKING!!!!")
